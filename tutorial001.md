...
Hent den fra ``||scene:Scene||``-menyen og plasser den under ``||variables:set minRobot to sprite of kind player||``.
Du må gjøre to endringer på blokken. Først må du klikke på ordet ``||variables:mySprite||`` og velge ``||minRobot||`` fra menyen som kommer fram.
Klikk så på det grå kvadratet etter ordet ``||scene:random||`` og velg det sandfargede kvadratet fra menyen som kommer opp.
